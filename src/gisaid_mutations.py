...
@license: GPL 3
@copyright: 2024_02_04

Note: This code contains a couple of big, ugly functions that will 
...
def GetArgs() -> argparse.Namespace:
    ...
    def ParseArgs(parser):
        ...
        parser.add_argument('-g', '--refseq_gb_file',
                            required = False,
                            help = 'GenBank file for Wuhan reference sequence. (default = ./NC_045512.gb)',
                            type = str,
                            default = './NC_045512.gb')
        parser.add_argument('-q', '--min_column_quality',
                            required = False,
                            help = 'Minimum quality to be accepted. Quality is the percent of A, C, G, T in column. (default = 0.9)',
                            default = 0.9,
                            type = float)
        parser.add_argument('-c', '--consensus_cutoff',
                            required = False,
                            help = 'The upper value for variation in an alignment column. (default = 0.99999)',
                            default = 0.99999,
                            type = float)
...
def read_alignment_file(filename: str) -> tuple[dict[str, SeqRecord], MultipleSeqAlignment]:
    """
...
def get_col_range(align: GISAIDData, min_qualityL: float = 0.99) -> tuple[int, int]:
    """
...
def ref_pos_to_alignment(ref_seq: SeqRecord) -> dict[int, int]:
    """
...
def format_qual(qualifiers: dict[str, str], id: str) -> str:
    """
...
def count_mutations(align: GISAIDData, pos_map: dict[int, int], ref_seq: SeqRecord,
                    start: int  = 130, end: int = 29840, consensus_cutoff: float = 0.8) -> dict:
    """
    ...
    """
    # helper function
    def remove_non_nucs(clist: tuple[np.bytes_, int]) -> tuple[np.bytes_, int]:
        """
    ...
    for col in range(start, end):
        ...
        if len(c) > 1:
            ...
            if align.align_array[0, col] == common[0][0]:
                continue
...
def find_location_feature(ref_seq: SeqRecord, position: int, alt: str, consens: str) -> dict:
    """
...
def count_the_mutations(ref_seq_gb_file: str,
                        msa_file: str,
                        ref_id: str,
                        min_col_quality: float = 0.90,
                        consensus_cutoff: float = 0.99999,
    ...
    ref_id : str
        The ID of teh reference sequence
