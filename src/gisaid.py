...
class GISAID_Data_Record(object):
    ...
    """
    def __init__(self, line: str, alignment_row: int) -> None:
        """
...
class GISAIDData(object):
    ...
    def __init__(self, filename: str) -> None:
        ...
        """
        def get_msa_size() -> tuple[int, int]:
            """
            ...
            Note
            ----
            This is faster than Bio.AlignIO
            """
...
def gisaid_read_msa(msa_file: str) -> MultipleSeqAlignment:
    """
...
def gisaid_format_date(date: str) -> datetime:
    """
...
def gisaid_read_alignment_file(filename: str) -> GISAIDData:
    """
...
def gisaid_get_col_range(col_variation: dict[int, Counter], seq_count: int, align_length: int,
                         min_quality:float = 0.99) -> tuple[int, int]:
    """
...
def gisaid_ref_pos_to_alignment(align_data: GISAIDData, ref_id: str) -> dict[int, int]:
    """
...
def get_col_variation(shm_name: str, shape: tuple[int, int], col: int, 
                      dtype:str = '|S1', order:str = 'F') -> tuple[int, Counter]:
    """
    ...
    col : int
        The column number.
    dtype: str
        dtype for shared memory
    order: str
        "F" or "R" forward or reverse

    Returns
    -------
    dictionary
        key is column number   
        value is a Counter object containing counts of each nucleotide.        
    """
...
def gisaid_get_columns_variation(alignment: GISAIDData, 
                                 num_processes: int = 10) -> dict[int, Counter]:
    """
    Get counts of each nucleotide type in each column of MSA.

    Parameters
    ----------
    alignment : GISAIDData
        An MSA object
    num_processes : int
...
def gisaid_get_varying_columns(align: GISAIDData, 
                               variation: dict[int, Counter],
                               consensus_cutoff: float = 1.0,
                               start: int = 130, end: int = 29840) -> dict[int, tuple[float, list[str]]]:
    """
    ...
    arguments:
    align - GISAIDDate
    variation: a dictionary returned by gisaid_get_columns_variation
